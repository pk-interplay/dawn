...
import "../lib/env";
// Reads through the transport's read-only helper rather than constructing its own SDK
// client: one AgentMail handle exists in this repo and it lives in agentmail.ts.
...
async function main() {
  const messages = await listInboxMessages(AGENTMAIL_INBOX_ID, LIMIT);
