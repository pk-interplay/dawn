...
| Multi-step campaigns with `steps jsonb`, `step_index`, `next_action_at`, and a state machine over `pgmq` (§6) | **Bounded follow-ups.** One ask, two nudges, then it dies. | Sequencing is where the workflow complexity lives — invisible queue messages, halting on reply, deploy-safe replay. ~~None of it is needed to ask an investor what they're deploying into. Add follow-ups when there is evidence a follow-up lifts reply rate.~~ **Amended (2026-08-12):** this was wrong, and wrong in a way that mattered. Under double opt-in, silence is the majority outcome of a first ask, so "one email, wait, done" made *give up quietly after seven days* the most-travelled path in the product. Migration 0036 adds the one piece that was genuinely missing — `next_action_at` plus a per-side counter on `introductions` — and `/api/cron/nudge-intros` sweeps it. `pgmq` and `steps jsonb` stay dropped: a fixed two-nudge sequence on a due-time column needs no queue, no step index, and no replay story. |
